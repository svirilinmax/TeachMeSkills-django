...
class ScheduleViewSetTest(TestCase):
    ...
    def test_api_filters_by_start_date(self):
        ...
        response = self.client.get(self.api_url, {"start_date": today.strftime("%Y-%m-%d")})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        data = response.json()['results']
    ...
    def test_api_filters_by_exact_date(self):
        ...
        response = self.client.get(self.api_url, {"exact_date": exact_date.strftime("%Y-%m-%d")})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        data = response.json()['results']
        self.assertEqual(len(data), 1)
        self.assertEqual(data[0]['movie_detail']['name'], "Paul Maul")
    ...
    def test_create_schedule_not_allowed(self):
        ...
        self.assertFalse(Schedule.objects.filter(movie_start=schedule_data["movie_start"]).exists())
